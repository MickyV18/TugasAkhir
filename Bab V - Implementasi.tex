...
Bagian ini menjelaskan spesifikasi perangkat keras dan perangkat lunak yang digunakan dalam tahap implementasi penelitian.

\subsection{Lingkungan Perangkat Keras}
Implementasi sistem ini dilakukan menggunakan perangkat keras dengan spesifikasi sebagai berikut:
\begin{enumerate}
	\item Prosesor: AMD Ryzen 5 6600H with Radeon Graphics (6 Core, 12 Thread)
	\item Memori (RAM): 16 GB
	\item Penyimpanan: SSD 1 TB
\end{enumerate}
...
Mengingat gambar-gambar dari sumber kedua (Mendeley Data) memiliki karakteristik yang berbeda dibandingkan sumber pertama, penelitian ini merancang tiga skenario eksperimen penempatan data sumber kedua untuk mengevaluasi dampaknya terhadap generalisasi model. Ketiga skenario tersebut adalah sebagai berikut:
\begin{enumerate}
	\item Skenario 1
	Data sumber kedua pada subset \textit{test} saja: Seluruh gambar dari sumber kedua ditempatkan eksklusif pada subset \textit{test}. Skenario ini bertujuan untuk mengukur kemampuan generalisasi model yang dilatih sepenuhnya pada data sumber pertama ketika diuji pada distribusi data yang berbeda.
...
\end{enumerate}
Ketiga skenario ini dirancang untuk mengkaji sejauh mana asal-usul dan penempatan data sumber kedua memengaruhi kinerja model klasifikasi, khususnya dalam konteks perbedaan distribusi (\textit{domain shift}) antara kedua sumber data.

Kondisi dataset mentah yang heterogen ini menjadi dasar dilakukannya proses \textit{preprocessing}.
...
\begin{enumerate}
	\item Konversi Mode Warna ke RGB\\
	Seluruh gambar yang tidak berformat RGB dikonversi menjadi mode warna RGB. Proses ini menyasar 263 gambar yang sebelumnya berformat L (\textit{grayscale}), RGBA, dan P. Setelah \textit{preprocessing}, 100\% gambar (557 gambar) telah berformat RGB secara seragam, sehingga kompatibel dengan arsitektur model yang mengharapkan \textit{input} tiga kanal warna.
	
	\item Standardisasi Resolusi ke $224 \times 224$ Piksel\\
	Seluruh gambar diubah ukurannya (\textit{resize}) menjadi $224 \times 224$ piksel, sesuai dengan ukuran \textit{input} standar pada model berbasis arsitektur seperti VGG, ResNet, dan EfficientNet. Sebelum \textit{preprocessing}, tidak ada satu pun gambar yang memiliki resolusi target tersebut. Setelah \textit{preprocessing}, seluruh 557 gambar telah memiliki resolusi yang seragam $224 \times 224$ piksel.
