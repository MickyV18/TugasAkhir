...
Tabel \ref{tab:statistik_roi} memperlihatkan dampak seleksi ROI terhadap karakteristik dataset. Jumlah citra tetap konstan (557 citra, perubahan 0{,}00\%), yang mengindikasikan bahwa mekanisme \textit{fallback} berhasil menangani seluruh kasus tanpa kontur \textit{valid}, sehingga tidak ada data yang hilang akibat proses \textit{cropping}. Rata-rata lebar, tinggi, dan resolusi piksel masing-masing menurun sebesar 21{,}59\%, 14{,}69\%, dan 30{,}40\%, yang menunjukkan bahwa area \textit{background} gelap pada citra CT Scan cukup dominan dan berhasil dieliminasi secara signifikan oleh proses \textit{thresholding} Otsu.
...
Setelah kedua metode \textit{preprocessing} diterapkan, dilakukan pengujian untuk melihat mana yang hasilnya lebih baik. Pengujian ini menggunakan beberapa metrik, yaitu PSNR, SSIM, AMBE, \textit{entropy}, dan \textit{edge density}. Untuk mempermudah penyebutan, skema CLAHE + Gaussian \textit{denoise} disebut GC, sedangkan skema \textit{Wavelet filter} + \textit{intensity normalization} disebut WN.
...
Berdasarkan Tabel \ref{tab:kualitas_gc_wn}, terlihat bahwa GC memiliki nilai PSNR dan SSIM yang lebih tinggi dibandingkan WN. Artinya, citra hasil GC lebih mirip dengan citra aslinya dan tidak terlalu banyak mengalami perubahan atau kerusakan pada strukturnya. Selain itu, nilai AMBE GC (5,3102) juga jauh lebih kecil dibandingkan WN (178,242). Nilai AMBE yang kecil ini menunjukkan bahwa tingkat kecerahan citra hasil GC tidak berubah jauh dari citra aslinya. Sebaliknya, nilai AMBE WN yang besar kemungkinan disebabkan oleh proses normalisasi \textit{z-score} yang memang bekerja dengan mengubah skala nilai piksel (dikurangi \textit{mean} lalu dibagi \textit{std}), sehingga tingkat kecerahannya jadi berbeda cukup jauh dari citra asli. Ini berbeda dengan CLAHE yang cara kerjanya membatasi kontras, sehingga kecerahan citra secara keseluruhan lebih terjaga.
...
Kemudian, pada Tabel \ref{tab:entropi_edge_density}, terlihat bahwa GC berhasil menaikkan nilai \textit{entropy} dari 5,0840 (sebelum \textit{preprocessing}) menjadi 5,9343. Nilai \textit{entropy} yang naik ini menandakan bahwa citra hasil GC memiliki lebih banyak detail atau informasi yang bisa dilihat, karena kontras lokalnya diperkuat oleh CLAHE. Berbeda dengan itu, WN justru membuat nilai \textit{entropy} turun menjadi 4,4025. Hal ini terjadi karena proses \textit{soft-thresholding} pada \textit{wavelet} memang berfungsi untuk menghilangkan derau, tapi efek sampingnya sebagian detail atau tekstur halus pada citra ikut berkurang.
...
Dari hasil ini, dapat disimpulkan bahwa kedua metode \textit{preprocessing} memiliki kelebihan masing-masing. GC lebih unggul dalam menjaga struktur citra tetap mirip dengan aslinya, menjaga kecerahan, dan menambah informasi detail citra. Sementara itu, WN lebih unggul dalam mempertahankan ketajaman garis tepi, meskipun kecerahan dan sebagian detail teksturnya berkurang.
...
Arsitektur model yang diimplementasikan pada penelitian ini secara umum mengikuti struktur backbone CNN yang dijelaskan pada artikel acuan ................. Kesesuaian tersebut terlihat pada aspek ukuran input 224×224×3, penggunaan enam blok konvolusi dengan kernel 3×3 dan padding "same", jumlah filter yang meningkat secara bertahap (8, 16, 32, 64, 128, 256), layer BatchNormalization dan AveragePooling setelah blok konvolusi terakhir, serta dua dense layer dengan 512 neuron yang identik dengan deskripsi pada artikel.
