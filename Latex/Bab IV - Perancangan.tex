...
\begin{enumerate}
	...
	Normalisasi intensitas tidak dilakukan pada tahap penyimpanan \textit{file} ini. Citra disimpan dalam format uint8 RGB $[0, 255]$ murni guna menjaga informasi kecerahan absolut antarcitra. Sebagai gantinya, statistik normalisasi global berupa nilai \textit{mean} dan \textit{standard deviation} ($\sigma$) per kanal dihitung secara \textit{streaming} dari seluruh \textit{train set} dan disimpan ke dalam berkas \texttt{norm\_stats.json}. Statistik ini dihitung hanya dari \textit{train set} untuk mencegah \textit{data leakage} dari data validasi maupun pengujian. Normalisasi kemudian diterapkan oleh \textit{dataloader} pada \textit{notebook} berikutnya menggunakan metode standardisasi (\textit{Z-score normalization}) dengan persamaan 
	matematis berikut:
	\begin{equation}
		X_{norm} = \frac{X / 255{,}0 - \mu}{\sigma}
	\end{equation}
	di mana $\mu$ adalah \textit{mean} dan $\sigma$ adalah \textit{standard deviation}	per kanal dari \textit{train set}. Proses ini memastikan normalisasi yang konsisten antara fase pelatihan (\textit{training}) dan inferensi (\textit{inference}), sekaligus menstabilkan pembaruan bobot (\textit{weight updates}) saat proses pelatihan model CNN.
